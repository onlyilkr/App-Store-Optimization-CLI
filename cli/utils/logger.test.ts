...
describe("logger utilities", () => {
  ...
  it("routes warn/error levels to stderr in console transports", () => {
    logger.setOutputModes([{ mode: "console", showErrorStack: false }]);
    const consoleTransport = (logger as any).winstonLogger.transports[0];
    ...
    const jsonTransport = (logger as any).winstonLogger.transports[0];
    expect(jsonTransport.stderrLevels).toMatchObject({
      warn: true,
      error: true,
    });
  });
