...
describe("aso-cache-local", () => {
  ...
  it("treats complete rows as cache hits regardless of difficulty/appCount combination", async () => {
    upsertKeywords("US", [
      {
    ...
    const result = await repository.getByKeywords({
      country: "US",
      keywords: ["competitive-fallback", "low-competition", "possible-real-one"],
    });

    expect(result.hits).toHaveLength(3);
    expect(result.hits.map((item) => item.keyword)).toEqual([
