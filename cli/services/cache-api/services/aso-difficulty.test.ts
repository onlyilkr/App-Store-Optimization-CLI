...
describe("aso-difficulty", () => {
  it("calculates per-app competitive score from explicit inputs", () => {
    ...
    expect(breakdown.score).toBeCloseTo(0.50519, 4);
    expect(breakdown.score100).toBeCloseTo(50.519, 2);
  });

  it("maps keyword match type to score centrally", () => {
    expect(keywordMatchToScore("titleExactPhrase")).toBe(1);
    expect(keywordMatchToScore("titleAllWords")).toBe(0.8);
    expect(keywordMatchToScore("subtitleExactPhrase")).toBe(0.5);
    expect(keywordMatchToScore("combinedPhrase")).toBe(0.4);
  ...
  it("calculates keyword difficulty using average, minimum, and app-count weights", () => {
    ...
    expect(difficulty.rawDifficulty).toBeCloseTo(0.4753, 5);
    expect(difficulty.difficultyScore).toBeCloseTo(47.5304, 4);
