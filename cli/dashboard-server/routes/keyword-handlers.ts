...
export function createKeywordHandlers(deps: AsoRouteDeps) {
  ...
  async function handleApiAsoKeywordsGet(
    res: http.ServerResponse,
    query: Record<string, string>
  ): Promise<void> {
    const country = normalizeCountry(query.country);
    const appId = query.appId;
    const difficultyEntitled = await asoBackendClient.isDifficultyEntitled();
    ...
    const withMeta = [...filtered, ...failedPlaceholders]
      ...
      .map((k) => {
        ...
        const { difficultyState: _difficultyState, ...baseKeyword } = k as typeof k & {
          difficultyState?: string;
        };
        return {
          ...baseKeyword,
          difficultyScore: difficultyEntitled ? k.difficultyScore : null,
